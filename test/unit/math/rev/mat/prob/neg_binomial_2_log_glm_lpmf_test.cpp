...
TEST(ProbDistributionsNegBinomialLog2GLM, test_scalar_stdvec_rowvec){
  ...
  Matrix<double, Dynamic, 1> beta(2, 1);
  beta << 0.3, 2;
  double alpha = 0.3; double phi = 2;
  EXPECT_NO_THROW(
                  stan::math::neg_binomial_2_log_glm_lpmf(y, x, alpha,
                                                          beta, phi));
  std::vector<double> y_vec(3);
  y_vec[0] = 0; y_vec[0] = 0; y_vec[0] = 0;
  EXPECT_NO_THROW(stan::math::neg_binomial_2_log_glm_lpmf(y_vec, x, alpha,
                                                          beta, phi));
  double y_scal = 1.0;
