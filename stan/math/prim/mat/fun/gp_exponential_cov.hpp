...
gp_exponential_cov(const std::vector<T_x> &x, const T_s &sigma,
  ...
  for (size_t i = 0; i < (x_size - 1); ++i) {
    ...
    for (size_t j = i + 1; j < x_size; ++j) {
      cov(i, j) =
          sigma_sq * exp(neg_inv_l * sqrt(squared_distance(x[i], x[j])));
      cov(j, i) = cov(i, j);
    }
  }
...
gp_exponential_cov(const std::vector<T_x> &x, const T_s &sigma,
  ...
  for (size_t i = 0; i < (x_size - 1); ++i) {
    ...
    for (size_t j = i + 1; j < x_size; ++j) {
      ...
      cov(i, j) =
          sigma_sq * exp(-1.0 * sqrt(squared_distance(x[i], x[j])) * temp);
      cov(j, i) = cov(i, j);
    }
  }
  cov(x_size - 1, x_size - 1) = sigma_sq;
...
gp_exponential_cov(const std::vector<T_x1> &x1, const std::vector<T_x2> &x2,
  ...
  for (size_t i = 0; i < x1_size; ++i) {
    for (size_t j = 0; j < x2_size; ++j) {
      cov(i, j) =
          sigma_sq * exp(neg_inv_l * sqrt(squared_distance(x1[i], x2[j])));
    }
  }
  return cov;
}
